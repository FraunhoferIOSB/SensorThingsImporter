...
public class DsMapperFilter implements DatastreamMapper {
    ...
    public Entity getMultiDatastreamFor(Tuple record, ErrorLog errorLog) {
        try {
            String filter = Translator.fillTemplate(filterTemplate, record, StringType.URL, true);
            Entity ds = getMultiDatastreamFor(filter);
            return ds;
        } catch (ServiceFailureException ex) {
            LOGGER.error("Failed to fetch datastream.", ex);
            throw new IllegalArgumentException(ex);
    ...
    private Entity getDatastreamFor(String filter, Tuple record, ErrorLog errorLog) throws ServiceFailureException, ImportException {
        ...
        if (streams.size() > 1) {
            ...
            LOGGER.debug("Found {} for filter {}.", ds, filter);
        }
        if (ds == null) {
    ...
    private Entity getMultiDatastreamFor(String filter) throws ServiceFailureException {
        ...
        LOGGER.info("Found {} for query {}.", mds, filter);
